...
//	}
	...
	//-----------迁徙图表优化开始----------------
	public List<String> getLgend(Collection<TMisMigrationData> list){
		 List<String>  lgend=new ArrayList<String>();
		 for (TMisMigrationData tMisMigrationData : list) {
			lgend.add(tMisMigrationData.getName());
		}
		 return lgend;
	}
	public List<Integer> getYxis(Collection<TMisMigrationData> list){
		List<Integer>  Yxis=new ArrayList<Integer>();
		int min=0,max=0,cyclenum=0;
		for (TMisMigrationData tMisMigrationData : list) {
			List<BigDecimal> data = tMisMigrationData.getData();
			for (BigDecimal object : data) {
				BigDecimal bigDecimal=object;
				if(cyclenum==0){
					min=bigDecimal.intValue();
					max=bigDecimal.intValue();
				}
				if(cyclenum!=0){
					if(bigDecimal.intValue()<min)
						min=bigDecimal.intValue();
					if(bigDecimal.intValue()>max)
						max=bigDecimal.intValue();
				}
				cyclenum++;
			}
	...
	public Map<String, Object> migrationGetData(TMisMigrationRateReport tMisMigrationRateReport) {
		...
		Collection<TMisMigrationData>  smdList1=new ArrayList<TMisMigrationData>();
		Collection<TMisMigrationData>  smdList2=new ArrayList<TMisMigrationData>();
		Collection<TMisMigrationData>  smdList3=new ArrayList<TMisMigrationData>();
		Collection<TMisMigrationData>  smdList4=new ArrayList<TMisMigrationData>();
		
		String  migrate= tMisMigrationRateReport.getMigrate();
		if("new".equals(migrate)){
			
			smdList1 = tMisMigrationRateReportService.getDataList(Migrate.cp1new, 5);
			smdList2 = tMisMigrationRateReportService.getDataList(Migrate.cp2new, 4);
			smdList3 = tMisMigrationRateReportService.getDataList(Migrate.cp3new, 3);
			smdList4 = tMisMigrationRateReportService.getDataList(Migrate.cp4new, 2);
		}
		if("corpus".equals(migrate)){
			
			smdList1 = tMisMigrationRateReportService.getDataList(Migrate.cp1corpus, 5);
			smdList2 = tMisMigrationRateReportService.getDataList(Migrate.cp2corpus, 4);
			smdList3 = tMisMigrationRateReportService.getDataList(Migrate.cp3corpus, 3);
			smdList4 = tMisMigrationRateReportService.getDataList(Migrate.cp4corpus, 2);
		}
		
		List<String>  qtime1=getLgend(smdList1);
		List<String>  qtime2=getLgend(smdList2);
		List<String>  qtime3=getLgend(smdList3);
		List<String>  qtime4=getLgend(smdList4);
		int min1=0,max1=0,min2=0,max2=0,min3=0,max3=0,min4=0,max4=0,split1=0,split2=0,split3=0,split4=0;
		
		List<Integer> yxis1=getYxis(smdList1);
		List<Integer> yxis2=getYxis(smdList2);
		List<Integer> yxis3=getYxis(smdList3);
		List<Integer> yxis4=getYxis(smdList4);
		min1=yxis1.get(0);
		max1=yxis1.get(1);
		min2=yxis2.get(0);
		max2=yxis2.get(1);
