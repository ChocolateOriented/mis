...
// 		});
		...
		$("#telstatus").change(function() {
			var selected = $("#telstatus").val();
			if(selected=="PTP"||selected=="PTPX"){
				$("div[name='promisepaydateGroup']").css("display", "block");
				$("#promisepaydate").val("");
			}else{
				$("div[name='promisepaydateGroup']").css("display", "none");
	 			$("#promisepaydate").val("");
			}
			if(selected=="ALPA"||selected=="FEE"||selected=="INSY"||selected=="KNOW"||
			selected=="MESS"||selected=="PTP"||selected=="PTPX"){
				$("#iseffective").val("1");
			}else{
				$("#iseffective").val("0");
			}
...
<%-- 				<form:input path="" htmlEscape="false" maxlength="10" class="input-xlarge required digits"/> --%>
		...
		<input style="display:none;" id="iseffective" name="iseffective" value="" />
