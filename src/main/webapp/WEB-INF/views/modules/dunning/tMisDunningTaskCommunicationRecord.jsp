...
<body>
	...
	<ul class="nav nav-tabs">
		<c:if test="${dunningCycle ne 'Q0'}">
	        ...
	        <shiro:hasPermission name="dunning:tMisDunningTask:view"><li  class="active"><a href="${ctx}/dunning/tMisDunningTask/communicationRecord?mobileSelf=${mobileSelf}&buyerId=${buyerId}&dealcode=${dealcode}&dunningtaskdbid=${dunningtaskdbid}&hasContact=${hasContact}&dunningCycle=${dunningCycle}">通话记录</a></li></shiro:hasPermission>
		</c:if>
        ...
        <shiro:hasPermission name="dunning:tMisDunningDeduct:view"><li><a href="${ctx}/dunning/tMisDunningDeduct/list?mobileSelf=${mobileSelf}&buyerId=${buyerId}&dealcode=${dealcode}&dunningtaskdbid=${dunningtaskdbid}&hasContact=${hasContact}&dunningCycle=${dunningCycle}">扣款信息</a></li></shiro:hasPermission>
	</ul> 
<!-- 	通话记录 -->
...
<%-- 					<input  name="btnCollection" contactMobile="${p.tel}" contactstype="COMMUNCATE" onclick="collectionfunction(this)"  class="btn btn-primary" method="Tel"  type="button" value="催收电话" /> --%>
	...
	<shiro:hasPermission name="dunning:tMisDunningTask:Commissionerview">
	<input id="btnSms"   name="btnCollection"  onclick="collectionfunction(this)" class="btn btn-primary" contactMobile="${personMobile}" contactstype="${dunningCycle eq 'Q0' ? 'SELF' : ''}"  method="Sms" type="button" value="催收短信" />
