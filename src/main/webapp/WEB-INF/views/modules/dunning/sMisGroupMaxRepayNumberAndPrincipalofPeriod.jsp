...
<html>
<head>
    <title>组周期还款最大户数&本金</title>
    <meta name="decorator" content="default"/>
    <script type="text/javascript">
        $(document).ready(function() {
            $("#btnExport").click(function(){
                top.$.jBox.confirm("确认要导出组周期还款最大户数&本金数据吗？","系统提示",function(v,h,f){
                    if(v=="ok"){
                        $("#searchForm").attr("action","${ctx}/dunning/tMisDunningMaxRepayNumberAndPrincipalReport/exportGroupMaxRepayNumberAndPrincipalListofPeriod");
                        $("#searchForm").submit();
                    }
                },{buttonsFocus:1});
        ...
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action","${ctx}/dunning/tMisDunningMaxRepayNumberAndPrincipalReport/groupMaxRepayNumberAndPrincipalofPeriod");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/dunning/tMisDunningMaxRepayNumberAndPrincipalReport/groupMaxRepayNumberAndPrincipalofPeriod">组周期还款最大户数&本金</a></li>
</ul>
